...
export function extractBrandingFromPage(): RawBrandingData {
	...
	const toPx = (value: string | null | undefined): number | null => {
		...
		if (value.endsWith("rem"))
			return (
				...
				Number.parseFloat(getStyle(document.documentElement).fontSize || "16")
			);
		if (value.endsWith("em"))
			return (
				Number.parseFloat(value) *
				Number.parseFloat(
					getStyle(document.body ?? document.documentElement).fontSize || "16",
				)
			);
	...
	const isHomeHref = (href: string | null | undefined): boolean => {
		...
		if (
			["", "/", "./", "/home", "/index", "/index.html", "#"].includes(trimmed)
		)
			return true;
	...
	const isValidBackgroundColor = (
		...
		if (rgba)
			return Number.parseFloat(rgba[1]!) >= constants.maxTransparentAlpha;
	...
	const isButtonElement = (el: Element | null): boolean => {
		...
		const hasButtonClasses =
			/rounded(-md|-lg|-xl|-full)?|p[xy]?-\d+|border.*rounded|inline-flex.*items-center.*justify-center/.test(
				classes,
			);
		const hasPadding =
			(Number.parseFloat(style.paddingTop) || 0) >
				constants.buttonMinPaddingVertical ||
			(Number.parseFloat(style.paddingBottom) || 0) >
				constants.buttonMinPaddingVertical ||
			(Number.parseFloat(style.paddingLeft) || 0) >
				constants.buttonMinPaddingHorizontal ||
			(Number.parseFloat(style.paddingRight) || 0) >
				constants.buttonMinPaddingHorizontal;
		const hasSize =
			rect.width > constants.buttonMinWidth &&
			rect.height > constants.buttonMinHeight;
		const hasShape =
	...
	const sampleElements = (): Element[] => {
		...
		push(
			'header img, header svg, nav img, nav svg, .site-logo img, img[alt*="logo" i], img[src*="logo" i]',
			20,
		);
		push(
			'button, input[type="submit"], input[type="button"], [role=button], [data-primary-button], [data-secondary-button], [data-cta], a.button, a.btn, [class*="btn"], [class*="button"], a[class*="bg-brand"], a[class*="bg-primary"], a[class*="bg-accent"]',
		...
		for (const link of Array.from(document.querySelectorAll("a")).slice(
			0,
			150,
		)) {
	...
	const getStyleSnapshot = (el: Element) => {
		...
		const isNavigation =
			...
			(/nav-|nav-|nav-link|sidebar|menu|toggle|trigger/.test(classes) ||
				el.matches(
					'[role="tab"],[role="menuitem"],[aria-haspopup],[aria-expanded]',
				) ||
				!!el.closest(
		...
		return {
			...
			typography: {
				...
				weight:
					Number.parseInt(style.getPropertyValue("font-weight"), 10) || null,
			},
	...
	const findImages = () => {
		...
		const collect = (el: Element, source: string) => {
			try {
				...
				if (
					/flag|language|locale/i.test(`${classes} ${id} ${alt}`) &&
					rect.width <= 32 &&
					rect.height <= 32
				)
					return;
				...
				if (
					!strongContext &&
					!/logo|brand/i.test(`${imgSrc} ${bgUrl || ""} ${alt}`)
				)
					return;
				...
				if (isSvg) {
					...
					const imageHref =
						...
						"";
					src = imageHref
						? resolveUrl(imageHref)
						: serializeSvg(el as SVGSVGElement);
					...
					if (el.querySelector("animate, animateTransform, animateMotion"))
						svgScore += 30;
				...
				const classMatch =
					hasLogoData || !!el.closest('[class*="logo" i], [id*="logo" i]');
				...
				logoCandidates.push({
					...
					href: href || undefined,
					source,
					logoSvgScore: finalSvg
						? src.startsWith("data:image/svg+xml")
							? Math.max(80, svgScore)
							: svgScore
						: 100,
				});
			} catch (error) {
		...
		for (const selector of backgroundSelectors) {
			queryAllDeep(selector).forEach((el) => {
				if (
					backgroundImageUrl(getStyle(el).getPropertyValue("background-image"))
				)
					collect(el, `background:${selector}`);
			});
		}
		...
		queryAllDeep("img").forEach((img) => {
			...
			if (
				...
				!img.closest(
					'[class*="testimonial" i], [class*="client" i], [class*="partner" i], footer',
				)
			)
		...
		queryAllDeep("svg").forEach((svg) => {
			...
			if (!existing) {
				...
				if (
					...
					svg.closest(
						'header, nav, [role="banner"], [class*="logo" i], [id*="logo" i]',
					)
				) {
		...
		queryAllDeep("a[href]")
			...
			.filter((el) => {
				...
				return (
					rect.top >= 0 &&
					rect.top < constants.topPageThreshold &&
					rect.width > 0 &&
					rect.height > 0
				);
			})
		...
		const best = (pickable.length ? pickable : unique).reduce<
			RawBrandingData["logoCandidates"][number] | null
		>((bestCandidate, candidate) => {
			...
			if (candidate.indicators.inHeader && !bestCandidate.indicators.inHeader)
				return candidate;
			if (candidate.indicators.hrefMatch && !bestCandidate.indicators.hrefMatch)
				return candidate;
			const area = candidate.position.width * candidate.position.height;
			const bestArea =
				bestCandidate.position.width * bestCandidate.position.height;
			if (
				area >= constants.minSignificantArea &&
				bestArea < constants.minSignificantArea
			)
				return candidate;
			return candidate.position.top < bestCandidate.position.top
				? candidate
				: bestCandidate;
		}, null);
	...
	const detectFrameworkHints = () => {
		...
		if (scripts.some((s) => /tailwind|cdn\.tailwindcss/.test(s)))
			hints.add("tailwind");
		if (scripts.some((s) => /bootstrap/.test(s))) hints.add("bootstrap");
		if (scripts.some((s) => /mui|material-ui/.test(s)))
			hints.add("material-ui");
		const classSample = Array.from(document.querySelectorAll("[class]"))
			.slice(0, 250)
			.map(classString)
			.join(" ");
		if (
			/\b(px|py|mx|my|bg|text|rounded|flex|grid|items-center|justify-)/.test(
				classSample,
			)
		)
			hints.add("tailwind-like");
		if (/\bbtn\b|\bcontainer\b|\brow\b|\bcol-/.test(classSample))
			hints.add("bootstrap-like");
		if (/Mui[A-Z]|chakra-|radix-|headlessui|react-aria/.test(classSample))
			hints.add("component-library");
	...
	const detectColorScheme = (): "dark" | "light" => {
		...
		const bg =
			getStyle(body ?? html).backgroundColor || getStyle(html).backgroundColor;
	...
	const getBackgroundCandidates = () => {
		...
		const add = (
			...
			if (normalized)
				candidates.push({ color: normalized, source, priority, area });
		};
		const bodyBg = getStyle(
		...
		if (
			mostCommon &&
			mostCommon !== normalizeColor(bodyBg) &&
			mostCommon !== normalizeColor(htmlBg)
		) {
		...
		for (const el of Array.from(
			...
			if (rect.width * rect.height > constants.minLargeContainerArea) {
				add(
					getStyle(el).backgroundColor,
					`${el.tagName.toLowerCase()}-container`,
					5,
