...
function detectVariants(candidates: LogoCandidate[]) {
	...
	for (const [index, candidate] of candidates.entries()) {
		...
		for (const [otherIndex, other] of candidates.entries()) {
			...
			const sameFile =
				filename(candidate.src) &&
				filename(candidate.src) === filename(other.src);
			const samePosition =
...
export function selectLogoWithConfidence(
	...
	if (!candidates.length) {
		return {
			...
			confidence: 0,
			source: "none",
			reasoning: "No logo candidates found",
		};
	}
	...
	for (const group of variants.values()) {
		...
		if (group.length > 1)
			variantBonus.set(best, (variantBonus.get(best) ?? 0) + 8);
	}

	const scored = candidates
		.map((candidate, index) => {
			if (!indicesToScore.has(index))
				return { index, score: -999, reasons: ["duplicate variant"] };
			...
			if (
				candidates.every(
					(other, otherIndex) =>
						otherIndex === index ||
						candidate.position.top <= other.position.top,
				)
			) {
				score += 12;
			...
			const square =
				Math.abs(candidate.position.width - candidate.position.height) < 5;
	...
	if (!scored.length) {
		return {
			...
			confidence: 0,
			source: "fallback",
			reasoning: "Only duplicate variants were found",
		};
	}
	...
	if (
		top.score >= confidence.strongScore &&
		separation >= confidence.strongSeparation
	) {
		return {
	...
	if (
		top.score >= confidence.goodScore &&
		separation >= confidence.goodSeparation
	) {
		return {
			selectedIndex: top.index,
			confidence: 0.75,
...
export function topLogoCandidates(
	candidates: LogoCandidate[],
	maxCandidates = 20,
) {
	if (candidates.length <= maxCandidates)
		return { candidates, indexMap: candidates.map((_, i) => i) };
	...
	return {
		candidates: top.map((x) => x.candidate),
		indexMap: top.map((x) => x.index),
	};
}
