...
function inferPalette(raw: RawBrandingData) {
	...
	for (const snapshot of raw.snapshots) {
		...
		bump(
			hexify(snapshot.colors.background, raw.pageBackground),
	...
	for (const color of raw.cssData.colors)
		bump(hexify(color, raw.pageBackground), 0.5);
	...
	const textPrimary =
		raw.colorScheme === "dark"
			? ranked.find((h) => h !== background && contrastYiq(h) > 180) ||
				"#FFFFFF"
			: ranked.find(
					(h) => h.toUpperCase() !== "#FFFFFF" && contrastYiq(h) < 160,
				) || "#111111";
	const chromatic = ranked.filter(
		(h) => isVibrant(h) && h !== background && h !== textPrimary,
	);
	const primary =
		chromatic[0] ||
		ranked.find(
	...
	const textSecondary =
		raw.colorScheme === "dark"
			? ranked.find(
					(h) => h !== textPrimary && contrastYiq(h) > 140 && isGrayish(h),
				)
			: ranked.find(
					(h) => h !== textPrimary && contrastYiq(h) < 180 && isGrayish(h),
				);
	...
	for (const candidate of [4, 6, 8, 10, 12]) {
		const share =
			normalized.filter(
				(v) =>
					v % candidate === 0 ||
					Math.abs((v % candidate) - candidate) <= 1 ||
					v % candidate <= 1,
...
function fontsFromStacks(stacks: string[][]) {
	...
	for (const stack of stacks) {
		for (const font of stack) {
			...
			if (!cleaned || ignored.test(cleaned) || /^var\(/i.test(cleaned))
				continue;
...
function representativeBorderRadius(
	radius?: StyleSnapshot["borderRadius"],
): string {
	const max = Math.max(
		radius?.topLeft || 0,
		radius?.topRight || 0,
...
function snapshotToButton(
	...
	return {
		...
		background:
			hexify(snapshot.colors.background, raw.pageBackground) || "transparent",
...
function extractButtons(raw: RawBrandingData): ButtonSnapshot[] {
		...
		.map((snapshot) => {
			...
			const border =
				snapshot.colors.borderWidth && snapshot.colors.borderWidth > 0
					? hexify(snapshot.colors.border, raw.pageBackground)
					: null;
			if (bg && bg !== "#FFFFFF" && bg !== "#FAFAFA" && bg !== "#F5F5F5")
				score += 300;
	...
	for (const item of candidates) {
		const bg =
			hexify(item.snapshot.colors.background, raw.pageBackground) ||
			"transparent";
		const border =
			item.snapshot.colors.borderWidth && item.snapshot.colors.borderWidth > 0
				? hexify(item.snapshot.colors.border, raw.pageBackground) ||
					"transparent-border"
				: "no-border";
		...
		seen.add(key);
		unique.push(
			snapshotToButton(item.snapshot, unique.length, raw, item.score),
		);
	}
...
function classifyButtons(buttons: ButtonSnapshot[]) {
	...
	const scored = buttons
		.map((button) => {
			...
			if (/primary|brand|cta|accent|bg-brand|bg-primary/i.test(button.classes))
				score += 250;
			if (
				/get started|sign up|start|try|buy|book|demo|download|join/i.test(
					button.text,
				)
			)
				score += 200;
	...
	const secondary = scored.find(
		(item) =>
			item.button.index !== primary?.index &&
			item.button.background !== primary?.background,
	)?.button;
...
export function processRawBranding(
	...
	const profile: BrandingProfile = {
		...
		typography: {
			fontFamilies: {
				primary: cleanFontName(
					raw.typography.stacks.body[0] || fonts[0]?.family || "system-ui",
				),
		...
		spacing: {
			...
			borderRadius: pickBorderRadius([
				...raw.snapshots.map((snapshot) => snapshot.radius),
				...raw.cssData.radii,
			]),
		},
		components: {},
		images: {
			logo:
				...
				null,
			favicon:
				raw.images.find((image) => image.type === "favicon")?.src || null,
			ogImage:
	...
	if (classified.primary)
		profile.components!.buttonPrimary = classified.primary;
	...
	profile.confidence = {
		...profile.confidence,
		overall:
			((profile.confidence?.logo ?? 0) +
				(profile.confidence?.colors ?? 0) +
				(profile.confidence?.buttons ?? 0)) /
			3,
	};
