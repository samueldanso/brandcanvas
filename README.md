...
2. **Permanent artwork delivery** — Every generated asset is pinned to IPFS and minted as an ERC-721 with a content hash. You get a viewable link, a token ID, and an on-chain explorer URL — proof of what was created, when, and by whom.
...
Claude generates brand identity + SVG artwork
        ↓
SVG pinned to IPFS (permanent viewable URL)
        ↓
Content hashed (keccak256), ERC-721 minted to payer on X Layer
        ↓
Buyer receives: viewUrl (IPFS art) + tokenId + explorerUrl + structured data
